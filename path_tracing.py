...
camera = rtu.Camera(
    ti.Vector([0, 1, -5], dt=ti.f32),
    ti.Vector([0, 0, 1], dt=ti.f32)
)
scene = rtu.Scene()

WALL_MAT = (rtu.MAT_DIFFUSE, ti.Vector([0.8, 0.8, 0.8]))
scene.add(rtu.Sphere(ti.Vector([0, -100.5, -1]), 100, WALL_MAT))
scene.add(rtu.Sphere(ti.Vector([0, 102.5, -1]), 100, WALL_MAT))
scene.add(rtu.Sphere(ti.Vector([0, 1.0, 101.0]), 100, WALL_MAT))
scene.add(rtu.Sphere(
    ti.Vector([-101.5, 0, -1]), 100, (rtu.MAT_DIFFUSE, ti.Vector([0.6, 0, 0])
)))
scene.add(rtu.Sphere(
    ti.Vector([101.5, 0, -1]), 100, (rtu.MAT_DIFFUSE, ti.Vector([0.0, 0, 0.6])
)))
